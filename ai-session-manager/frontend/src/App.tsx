        ...
        {selectedId ? (
          ...
          <>
            <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)', background: '#161922' }}>
              {(['overview', 'costs', 'config'] as const).map((t, i) => (
                <button key={t} onClick={() => setHomeTab(t)} style={{
                  ...
                  {t === 'costs' ? 'Cost Tracker' : t === 'config' ? 'AI Config' : 'Overview'}<kbd style={{ fontSize: 9, fontFamily: 'monospace', color: 'rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 3, padding: '1px 4px', marginLeft: 4, lineHeight: 1.5 }}>{i + 1}</kbd>
                </button>
              ))}
            </div>
